...
#include "D:\Usuario\Escritorio\Sistemas Embebidos\Laboratorio 3\Prueba\teclado_driver2probando.cpp"
...
void loop(){
  //delay(100) en el código del slave_sender ponian un delay de 100
  if (midiendo_lux == 1){
    adc_loop();
    //midiendo_lux=0;
  }else{
...
void crearMensaje(){
   ...
   switch(tipoMsj){
      ...
      case OBTENER_TODO:
      {
        ...
        msj[14] = (uint8_t) parteEntera; //PREGUNTAR SI NO ES CON DOBLE &&    y preguntar si el casteo esta bien
...
void descomprimirMensaje(uint8_t * mensaje){ 
  ...
  while(fin == 0){
        if(mensaje[i] == '[' && i==0){//lei el primer caracter
              pkt.tipo = mensaje[i+1]; //duda, notengo que formar el tipo con i+1,i+2,i+3 e i+4? porque es un entero y ocupa 4 bytes
        }             //no hace falta, porque es un entero menor a 256, ojo igual hay que ver que onda con esto
         if(i > 18){
        //hay código basura
            printf("formato de mensaje erróneo");
            fin=1;
        ...
        if(esPayload == 1){
                ...
                if(esTamanio == 1){
                              if(mensaje[i] == ','){ //es la segunda coma
                                ...
                                pkt.tamanio = mensaje[i]; //duda, notengo que formar el tamanio con i+1,i+2,i+3 e i+4? porque es un entero y ocupa 4 bytes
                              }               //no hace falta, porque es un entero menor a 256, ojo igual hay que ver que onda con esto
                }
...
void receiveEvent(int howMany) {
  ...
  if(Wire.available() == 5){
      ...
      if(tipo == TECLA_UP){
        mi_handler_down_tecla_0();
      }
      if(tipo == TECLA_DOWN){
        mi_handler_down_tecla_1();
      }
      if(tipo == TECLA_LEFT){
        mi_handler_down_tecla_2();
      }
      if(tipo == TECLA_RIGHT){
        mi_handler_down_tecla_3();
      }
      if(tipo == BOTON_A2){
        mi_handler_down_tecla_5();
      }
  }else{
...
void requestEvent() {
  ...
  if((int) pkt.tipo == OBTENER_TODO){
    Wire.write(msj,17); //agregar el strle por las duda
  }else{
    Wire.write(msj,8); //agregar el strle por las duda
  }
}
