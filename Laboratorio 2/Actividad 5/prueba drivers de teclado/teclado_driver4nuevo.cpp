...
void mi_callback(){ //esta funcion de callback especifica para el LDR nos convierte el valor  
	...
	Serial.println("estoy en la callback de teclado y el valor leido por adc es " + adc.adc_value);
...
void teclado_init(){
		...
		sei();
		Serial.println("inicializo el teclado, estoy en teclado init\n");
	
}
void teclado_loop(){
	
	//Serial.println(String(buttonInterrupt) + " <--buttonInterrupt");
	Serial.println("estoy en teclado loop\n");
