...
//#include "D:\Usuario\Escritorio\Sistemas Embebidos\Laboratorio 2\Actividad 5\prueba drivers de teclado\teclado_driver2 (mio).cpp"
...
void mi_handler_down_tecla_2(){ //esta fncion de callback especifica para el LDR nos convierte el valor  
  ...
  //Serial.println("entre izquierda");
  disminuirModo();
...
void mi_handler_up_tecla_2(){ //esta fncion de callback especifica para el LDR nos convierte el valor  
  ...
  // Serial.println("entre a up left desde la callback");
...
void setup() {
    ...
    // turn on CTC mode:
    TCCR0A |= (1 << WGM01); //cambiado el timer a 01, que se corresponde al timer 0
    // Set CS10 and CS12 bits for 1024 prescaler: If we set CS10 and CS12 using TCCR1B |= (1 << CS10); and TCCR1B |= (1 << CS12);, we divide our clock source by 1024
    TCCR0B |= (1 << CS10);
    TCCR0B |= (1 << CS12);
...
void loop(){
  if (midiendo_lux == 1){  
     //Serial.println("estoy en el if de midiendo lux==1, osea pasa a hacer adc_loop\n");
    adc_loop();
  }else{
    //Serial.println("estoy en el else de midiendo lux==1, osea pasa a hacer teclado loop\n");
    teclado_loop();
